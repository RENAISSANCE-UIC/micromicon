...
if (!exists("len_bp", inherits = FALSE)) {
  len_bp <- function(x) {
...
head(get_genome_metadata(genome))

# Access features using accessor
head(get_features(genome))

# Access sequences using accessor
get_contig_names(genome)
```
...
cat("Gene length:", len_bp(ampC_dna), "bp\n")
cat("Flanking:", flank_size * 2, "bp\n")
```

## BLASTP Analysis (REQUIRES local databse)
...
**Important**: Exporting from GenBank to GFF3+FASTA loses metadata (organism, taxonomy, references). This is a **one-way conversion** - you cannot convert GFF3+FASTA back to GenBank format. Always keep your original GenBank files!
...
region_seq <- extract_by_coords(genome, "MICR_AMPC", 1, 1000)
```
